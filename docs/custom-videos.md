...
The same `channel/` engine also has three other modes: `behind_the_business` (**How They Really Make Money** on YouTube; **800–2500** words) in `docs/behind-the-business.md`, `how_they_took_over` (**How They Took Over**; **800–2500** words, ~5–15 minutes) in `docs/how-they-took-over.md`, and `wealth_pov` (**Quiet Wealth**; **5,600–5,800** words, ~32–36 minutes, second-person life POV) in `docs/wealth-pov.md`. Do **not** apply those business, takeover, or Quiet Wealth rules to a What They Really Think title. Pass `--channel` explicitly; do not guess the channel from the title. Cloud / parallel runs use `python -m channel generate` and write `artifacts/<JOB_ID>/`. `DO NOT MODIFY THE VIDEO ENGINE` during a normal generation task.
...
The LangGraph HITL pipeline in `graph/` is a different product (fictional rank-POV videos). Do not route these titles through `ideate` — that node blocks real named people. This path is `channel/` → `script.txt` → Gemini TTS or imported audio → pause scenes → Nano Banana 2 or Google Flow stills → optional Omni 1.1 Flash (`gemini-omni-1.1-flash`) clips → optional Lyria 3 (`lyria-3-clip-preview`) bed → FFmpeg.
...
| Lyria 3 (`lyria-3-clip-preview`) | Opt-in instrumental background bed. `python -m channel music` after QA. Writes `audio/music.mp3` (30s loop; `lyria-3.5` for a longer song). Assemble mixes it quietly under the voiceover. Does not run on `--resume`. |
...
| `python -m channel assemble` | Drop-folder filename clocks, or pause-timed still holds (or Omni clips in `videos/` when present), + mux imported VO. Mixes `audio/music.*` under the voiceover when present. Drop-folder cuts assemble without burned captions. |
...
TITLE
  ...
  → python -m channel music (optional Lyria 3 / lyria-3-clip-preview)
...
15. Optional: `python -m channel music <JOB_ID>` (Lyria 3 / `lyria-3-clip-preview`, needs GEMINI_API_KEY)
    Instrumental background bed (no vocals). Writes `audio/music.mp3`.
    Default is a 30-second loop; `--full` uses `lyria-3.5`. Does not run on `--resume`.
    Skip this step to keep narration-only audio. `--no-music` on assemble ignores a bed.

16. python -m channel assemble <JOB_ID>
...
17. Verify: ffprobe duration + resolution; sync.max_cut_error_ms < 20;
...
18. Update docs/videos/<slug>.md + README.md with a "## Do not copy" list
...
- **Music (opt-in):** `python -m channel music` (Lyria 3, model `lyria-3-clip-preview`, 30-second instrumental loop; `--full` uses `lyria-3.5`). Writes `audio/music.mp3`. Assemble mixes it quietly under the voiceover. Same Gemini key. Does not run on `--resume`. Do not fall back to Suno or Udio. Operator-dropped `audio/music.*` or `drop/music.*` still mix without the API.
