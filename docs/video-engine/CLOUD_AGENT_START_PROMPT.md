...
Then fill research, story, and narration in `artifacts/<JOB_ID>/project.json` using the **master prompt** (`MASTER`) in the prompt module named in the manifest. Same staged loop on every channel; DNA customized per mode. Resume with `--resume <JOB_ID>`. If `GEMINI_API_KEY` is set, `python -m channel tts` synthesizes `gemini-3.1-flash-tts-preview` and saves narration chunks; `--resume` does that automatically. Otherwise stop at `WAIT_AUDIO`. Print the `script.txt` path. Do not write scenes before timestamps exist. After timestamps, deliver Google Flow prompts in batches of 20 and wait for **Reply "next"**. Do not emit `flow_prompts` until originality_score ≥ 80 and ready_to_publish. Then `python -m channel images` (Nano Banana 2 / `gemini-3.1-flash-image`) or paste into Google Flow. Optional `python -m channel videos` (`gemini-omni-1.1-flash`) after stills exist — do not run it on `--resume`. Optional `python -m channel music` (`lyria-3-clip-preview`) after QA — do not run it on `--resume`.
