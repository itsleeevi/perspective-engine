...
Wikipedia (seed), company IR / SEC sites (agent research), Gemini TTS and Nano Banana 2 stills (`generativelanguage.googleapis.com`) when `GEMINI_API_KEY` is set. No hidden model “latest”. Operator Google Flow stills remain valid. Voice is Gemini TTS or imported audio.
