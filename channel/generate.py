...
NEXT_STEPS = (
    ...
    "Paste flow_prompts.txt into Google Flow, or python -m channel images {job_id} (Nano Banana 2). Then ingest-images. Optional python -m channel videos {job_id} (gemini-omni-1.1-flash). Optional python -m channel music {job_id} (lyria-3-clip-preview). Then assemble.",
    IMAGE_FILENAME_RULE,
    ...
    *,
    ...
    if images_complete(dest):
        ...
        manifest.notes = [
            ...
            f"(gemini-omni-1.1-flash). Optional music: python -m channel music "
            f"{job_id} (lyria-3-clip-preview). Assemble: python -m channel assemble {job_id}",
        ]
    else:
        manifest.notes = [
            f"Nano Banana 2: python -m channel images {job_id}  "
            f"(needs GEMINI_API_KEY; writes images/nano_banana/ then ingest-images)",
            f"Or paste {dest / 'flow_prompts.txt'} into Google Flow, then: "
