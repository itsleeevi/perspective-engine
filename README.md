...
**Production YouTube documentaries** are the `channel/` engine: still-first 2D animated cuts for four named channels, assembled locally with Cursor GenerateImage, Kokoro, and FFmpeg.
...
Working contract for any agent with empty chat history: [`AGENTS.md`](AGENTS.md). Channel playbooks: [`docs/custom-videos.md`](docs/custom-videos.md), [`docs/behind-the-business.md`](docs/behind-the-business.md), [`docs/how-they-took-over.md`](docs/how-they-took-over.md), [`docs/wealth-pov.md`](docs/wealth-pov.md). Engine specs: [`docs/video-engine/`](docs/video-engine/).
...
Python ≥ 3.13, `ffmpeg` on `PATH` for assemble. Documentary generation does **not** need `FAL_KEY` or `ELEVENLABS_API_KEY`. Gemini TTS and Nano Banana 2 stills use `GEMINI_API_KEY`. If stills are missing, stop (or run `python -m channel images`).
...
Shipped cuts live in `fixtures/` plus a page under `docs/videos/`, `docs/business/`, `docs/takeover/`, or `docs/wealth/`. Do not commit `.mp4` files. Parallel Cloud jobs must not clobber repo-root `fixtures/`.
...
Copy `.env.example` to `.env` for `GEMINI_API_KEY` (channel TTS + Nano Banana 2 stills) or if you are exercising the graph adapters (Anthropic, fal.ai, ElevenLabs, `--image-provider nano-banana-2`). The graph path is independent of documentary production.
