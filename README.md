...
Python ≥ 3.13, `ffmpeg` on `PATH` for assemble. Documentary generation does **not** need `FAL_KEY` or `ELEVENLABS_API_KEY`. Gemini TTS uses `GEMINI_API_KEY`. If Google Flow stills are missing, stop.
...
Copy `.env.example` to `.env` for `GEMINI_API_KEY` (channel TTS) or if you are exercising the graph adapters (Anthropic, fal.ai, ElevenLabs). The graph path is independent of documentary production.
